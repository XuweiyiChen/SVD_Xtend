...
class StableVideoDiffusionPipeline(DiffusionPipeline):
    ...
    def __call__(
        self,
        first_image: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        last_image: Union[PIL.Image.Image, List[PIL.Image.Image], torch.FloatTensor],
        ...
        self.check_inputs(first_image, height, width)

        # 2. Define call parameters
        if isinstance(first_image, PIL.Image.Image):
            batch_size = 1
        elif isinstance(first_image, list):
            batch_size = len(first_image)
        else:
        ...
        image_embeddings = self._encode_image(
            last_image, device, num_videos_per_prompt, do_classifier_free_guidance
        )
        ...
        first_image = self.image_processor.preprocess(
            first_image, height=height, width=width
        )
        noise = randn_tensor(
            first_image.shape,
            generator=generator,
        ...
        first_image = first_image + noise_aug_strength * noise
        ...
        image_latents = self._encode_vae_image(
            first_image, device, num_videos_per_prompt, do_classifier_free_guidance
        )
