...
class DummyDataset(Dataset):
    def __init__(
        ...
        self.img_types = (".jpg", ".jpeg", ".png", ".bmp", ".gif")
        self.actions = actions
        if train_data is None:
            self.train_data = self.load_from_dir(hdf5_path)
        else:
            self.train_data = train_data

    def get_prompt(self, dataset_name):
        if dataset_name == "can_ph":
            return "Pick up the red can and place it in the bottom right container."
        elif dataset_name == "lift_ph":
            return "Pick up the red block."
        elif dataset_name == "square_ph":
            return "Pick up the square tool and place it on the square peg."

    def load_from_dir(self, hdf5_path):
        train_data = []
        for dataset_filename in os.listdir(hdf5_path):
            dataset_path = os.path.join(hdf5_path, dataset_filename)
            dataset_name = os.path.splitext(os.path.basename(dataset_filename))[0]

            if dataset_name in self.actions:
                print(f"loading data from {dataset_name}...")
                train_data.extend(self.build_hdf5(dataset_path, dataset_name))

        return train_data

    def build_hdf5(self, json_data, dataset_name):
        extended_data = []
        f = h5py.File(json_data, "r")
        demos = list(f["data"].keys())
        cam_views = ["agentview"]
        # rgb = [[] for _ in range(len(demos))]
        # depth = [[] for _ in range(len(demos))]
        prompt = self.get_prompt(dataset_name)
        for i, demo in enumerate(demos):
            if i == self.num_samples_max:
                break
            demo_grp = f["data/{}".format(demo)]
            for _, cam_view in enumerate(cam_views):
                if len(cam_views) == 1:
                    cam_view = cam_views[0]
                    rgb_seqs = list(demo_grp["obs/{}".format(cam_view + "_image")])
                    # depth_seqs = list(demo_grp["obs/{}".format(cam_view + "_depth")])
                    if len(rgb_seqs) > self.num_frames:
                        sample_idx = np.linspace(
                            0, len(rgb_seqs) - 1, self.num_frames, dtype=int
                        )
                        rgb_seqs = list(
                            demo_grp["obs/{}".format(cam_view + "_image")][sample_idx]
                        )
                        # depth_seqs = list(
                        #     demo_grp["obs/{}".format(cam_view + "_depth")][sample_idx]
                        # )
                        extended_data.append({"images": rgb_seqs, "lang": prompt})

        if len(extended_data) > self.num_samples_max:
            print(
                "num_samples_max is bigger than the entire dataset, "
                + "increase it to {}".format(len(extended_data))
            )
            self.num_samples_max = len(extended_data)

        return extended_data

    def extract_samples(self, indices):
        # Extract samples by the provided indices
        extracted_data = [self.train_data[idx] for idx in indices]
        new_dataset = DummyDataset(
            hdf5_path="",  # Set to an appropriate value or handle differently
        ...
        # new_dataset.train_data = extracted_data
        return new_dataset

    def remove_samples(self, indices):
        # Remove samples by the provided indices
        remaining_data = [
            self.train_data[idx]
            for idx in range(len(self.train_data))
            if idx not in indices
        ]
        self.train_data = remaining_data
        self.num_samples_max = len(self.train_data)

    def __len__(self):
        return min(len(self.train_data), self.num_samples_max)

    def __getitem__(self, idx):
        data_point = self.train_data[idx]
        rgb_images = data_point["images"]
        # description = data_point["lang"]

        pixel_values = torch.empty(
            (self.num_frames, self.channels, self.height, self.width)
        )
        for i, frame in enumerate(rgb_images):
...
def main():
    ...
    try:
        ...
        os.makedirs(args.out_dir_val, exist_ok=True)
        # Additional code for handling config can be added here if necessary
    except OSError as e:
    ...
    # Define the number of data points to split
    num_val_samples = args.num_validation_images

    # Generate random indices for validation samples
    val_indices = np.random.choice(len(train_dataset), num_val_samples, replace=False)

    # Create the validation dataset
    val_dataset = train_dataset.extract_samples(val_indices)

    # Update the training dataset by removing the validation samples
    train_dataset.remove_samples(val_indices)
